...
- **AI verbs** (`text`, `image`, `speech`, `transcription`): with no default and no `--provider` flag, marmot runs first-run auto-config — it detects available API keys in your env, picks the first ready provider in pecking order (`ollama` → `openrouter` → `vercel` → `cloudflare` → `openai` → `anthropic`; image/speech/transcription skip `ollama` and `anthropic`), persists the choice to this file, and continues the call. A one-line stderr note announces it.
