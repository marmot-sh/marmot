...
`lookup`-mode presets accept `type`, `q`, `limit`, `cursor`, `title`, `seniority`, `location`, `domain`, `industry`, `employees`, `tech`, `emailType`, `department`, `company`, `cache`, `refresh`, `output`, `raw`, `retries`, `timeout`, `session`. All filter values are scalar-replace. New negation flags: `--cache`, `--no-refresh`, `--no-raw`. See [Presets — Merge rules](/docs/reference/commands/presets#merge-rules).
