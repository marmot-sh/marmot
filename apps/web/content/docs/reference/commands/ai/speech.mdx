...
| `-o, --output <path>` | Output audio path. With `-o` set on a TTY, the path-print on stdout is suppressed; auto-playback still happens unless `--quiet`. See [Stdout decision matrix](/docs/reference/commands/common-flags#stdout-decision-matrix). |
