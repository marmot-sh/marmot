...
function addPresetWriteOptions(command: Command): Command {
  return command
    ...
    .option('-o, --output <path>', 'Default output path for this preset.')
    .option('--session <name>', 'Default session binding for this preset.')
    .option('--prompt-file <path>', 'Read prompt text from a file (text/image/speech/video modes).')
    // Text
    ...
    .option('--binary', 'Default to raw bytes on stdout (image / speech / video mode).')
    .option('--no-binary', 'Default to file output (overrides preset binary true; image / speech / video mode).')
    .option('--b64', 'Default to base64 envelope (image / speech / video mode).')
    .option('--no-b64', 'Disable b64 envelope default (image / speech / video mode).')
    .option('--preview', 'Enable inline image preview default (image mode).')
    .option('--no-preview', 'Disable inline image preview default (image mode).')
    // Speech
...
export function createProgram(): Command {
  ...
  const imageCommand = new Command('image')
    ...
    .option('--no-binary', 'Disable raw-bytes-to-stdout (overrides a preset that sets binary true).')
    .option('--b64', 'Emit JSON envelope with base64 image data inline.')
    .option('--no-b64', 'Disable b64 envelope (overrides a preset that sets b64 true).')
    .option('--json', 'Emit JSON envelope on stdout. Default prints just the file path(s).')
    .option('--no-json', 'Disable JSON envelope (overrides a preset that sets json true).')
    .option('--preview', 'Enable inline image preview (overrides a preset that sets preview false).')
  ...
  const speakCommand = new Command('speak')
    ...
    .option('--no-play', 'Disable speaker playback (overrides a preset that sets play true).')
    .option('--wait', 'When combined with --play, block until playback finishes (useful in scripts).')
    .option('--no-wait', 'Disable wait (overrides a preset that sets wait true).')
    .option('--retries <count>', 'Retry failed provider calls up to N times (default: 0).')
    .option('--timeout <seconds>', 'Per-attempt generation timeout in seconds (default: 120).')
    .option('--preset <name>', 'Apply a saved preset as defaults (explicit flags still win). Shorthand: @name.')
    .option('--session <name>', 'Bind this call to a session for logging.')
