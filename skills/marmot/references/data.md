...
- **Session binding (0.6.0+).** Every data verb accepts `--session <name>`. The bound name flows into the usage record so `marmot usage --session <name>` filters work on data traffic, and the call appears under `marmot session get <name>`. Pre-0.6.0 data verbs hardcoded `session: null` even when a session was active.
